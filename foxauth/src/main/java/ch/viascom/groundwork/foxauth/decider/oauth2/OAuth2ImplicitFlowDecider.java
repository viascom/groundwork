...
@NoArgsConstructor
@AllArgsConstructor
public class OAuth2ImplicitFlowDecider implements FoxAuthDecider {

    private int weight = 50;

    @Override
    public AuthValidatorType getAuthValidatorType() {
        return AuthValidatorType.OAUTH2_IMPLICIT_FLOW;
    }

    @Override
    public boolean isValid(FoxAuthHttpServletRequestWrapper servletRequestWrapper, FoxAuth foxAuth) {
        return OAuth2TypeChecker.hasType(servletRequestWrapper, "response_type", "token");
    }
}
